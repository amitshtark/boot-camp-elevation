...
function checkInventory(items) {
  return new Promise((resolve, reject) => {    
    setTimeout(() => {
      ...
      {
        if(!inventory[item])
          return reject(new Error(`${item} is not an item in our inventory`));
        if(inventory[item].stock === 0)
          return reject(new Error(`${item} is not in stock`));
      }
      resolve(items);
    }, 500);
  });
...
function calculateTotal(items) {
  ...
  setTimeout(() => {
    ...
    resolve({subtotoal: sum, tax: "8%", total: sum*1.08});
  }, 200);
  });
  // TODO: Return a promise that:
  // 1. Waits 200ms
...
function processPayment(amount) {
  ...
  setTimeout(() => {
    if (Math.random() < 0.1) 
      reject(new Error(`Failed to process payment(amount: ${amount})`));
    else
    {
      resolve({transactionId: Math.random(), amount: amount, status: 'success'});
    }
  }, 1500);
  });
  // TODO: Return a promise that:
...
function checkout(itemNames) {
  ...
  .then(result => processPayment(result.total))
  .then(() => updateInventory(itemNames))
  .then(finalInventory => ({ status: 'completed', inventory: finalInventory }));
